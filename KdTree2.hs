...
-- This module is a modified version of the one forked from here (version 0.2.1):
-- https://github.com/ijt/kdtree.git
...
----- $$$$$$$$$$ -----           
-- |finds the minimum point given some reference axis         
findMinimum :: (Eq p, Point p) => KdTree p -> Int -> Maybe p
findMinimum KdEmpty _ = Nothing
findMinimum (KdNode l p r axis) axis'
    |axis' < 0 = error "cannot use negative axis for reference"
    |axis' > dimension p = error "reference axis is higher than point dimension"
findMinimum (KdNode l p r axis) axis' = if axis == axis'
    then case findMinimum l axis' of Nothing -> Just p
                                     Just p' -> Just p'
    else let cand1 = case findMinimum l axis' of Nothing -> []
                                                 Just p' -> [p']
             cand2 = case findMinimum r axis' of Nothing -> []
                                                 Just p' -> [p']
         in Just $ L.minimumBy (compare `on` coord axis') $ cand1 ++ [p] ++ cand2

findMinimum' :: (Eq p, Point p) => KdTree p -> Int -> p
findMinimum' tree axis = L.minimumBy (compare `on` coord axis) $ (toList tree)

findMaximum :: (Eq p, Point p) => KdTree p -> Int -> Maybe p
findMaximum KdEmpty _ = Nothing
findMaximum (KdNode l p r axis) axis'
    |axis' < 0 = error "cannot use negative axis for reference"
    |axis' > dimension p = error "reference axis is higher than point dimension"
findMaximum (KdNode l p r axis) axis' = if axis == axis'
    then case findMaximum r axis' of Nothing -> Just p
                                     Just p' -> Just p'
    else let cand1 = case findMaximum l axis' of Nothing -> []
                                                 Just p' -> [p']
             cand2 = case findMaximum r axis' of Nothing -> []
                                                 Just p' -> [p']
         in Just $ L.maximumBy (compare `on` coord axis') $ cand1 ++ [p] ++ cand2

remove1 :: (Eq p, Point p) => KdTree p -> p -> KdTree p
remove1 KdEmpty _ = KdEmpty
remove1 (KdNode l p r axis) pKill =
    if p == pKill
        then case findMinimum r axis of Nothing -> case findMaximum l axis of Nothing -> KdEmpty
                                                                              Just p' -> KdNode (remove2 l p') p' r axis
                                        Just p' -> KdNode l p' (remove2 r p') axis                             
        else if coord axis pKill <= coord axis p
                then KdNode (remove2 l pKill) p r axis
                else KdNode l p (remove2 r pKill) axis
                
remove2 :: (Eq p, Point p) => KdTree p -> p -> KdTree p
remove2 KdEmpty _ = KdEmpty
remove2 (KdNode l p r axis) pKill =
    if p == pKill
        then case findMaximum l axis of Nothing -> case findMinimum r axis of Nothing -> KdEmpty
                                                                              Just p' -> KdNode l p' (remove1 r p') axis
                                        Just p' -> KdNode (remove1 l p') p' r axis                             
        else if coord axis pKill <= coord axis p
                then KdNode (remove1 l pKill) p r axis
                else KdNode l p (remove1 r pKill) axis                
----- $$$$$$$$$$ ----- 
...
insert :: (Point p) => KdTree p -> p -> KdTree p
insert KdEmpty point = fromList [point]
insert (KdNode l p r axis) point = if coord axis p >= coord axis point 
	then KdNode (insert l point) p r axis
	else KdNode l p (insert r point) axis